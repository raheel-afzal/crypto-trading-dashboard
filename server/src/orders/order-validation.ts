...
export const orderRequestSchema = z.strictObject(
  {
    ...
    price: z
      .number({ error: "price must be a finite number" })
      .gt(0, { error: "price must be a positive number" }),
  },
  {
    error: (issue) =>
      issue.code === "invalid_type" ? "order must be a JSON object" : undefined,
  },
);
...
export function checkOrder(
  order: OrderRequest,
  { marketPrice, cashBalance, heldQuantity }: OrderContext,
): OrderRejection | null {
