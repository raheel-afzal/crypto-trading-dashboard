...
const SERIES = 'var(--color-chart-line)';
...
export function PriceChart() {
  ...
  const coinTabs = (
    <div role="group" aria-label="Chart asset" className="flex gap-1">
      {coins?.map((item) => (
        <button
          key={item.symbol}
          type="button"
        ...
        <div className={`transition-opacity ${isPlaceholderData ? 'opacity-50' : ''}`}>
          {coin && (
            <p className="mb-3 flex items-baseline gap-3">
              ...
              <span className={`text-sm ${coin.change24h >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <AreaChart responsive data={history} margin={{ top: 8, right: 0, bottom: 0, left: 0 }} className="h-60 w-full">
            ...
            <Tooltip
              ...
              content={({ active, payload, label }) => {
                ...
                return (
                  <div className="rounded-md border border-white/10 bg-neutral-950 px-3 py-2 text-xs shadow-lg">
                    <p className="flex items-center gap-2 font-semibold tabular-nums text-neutral-50">
                      <span aria-hidden="true" className="h-0.5 w-3 rounded-full bg-chart-line" />
                      {formatUsd(price)}
                    </p>
            ...
            />
            {last && <ReferenceDot x={last.timestamp} y={last.price} r={4} fill={SERIES} stroke={SURFACE} strokeWidth={2} />}
          </AreaChart>
